...
@MODELS.register()
class UMT_contrast_1(nn.Module):
    ...
    def __init__(self,
                 ...
                 query_dec=None,
                 pred_head=None):
        super(UMT_contrast_1, self).__init__()
        ...
        self.optic_enc = build_model(optic_enc)
    ...
    def forward(self, data, mode):
        ...
        if self.optic_enc is not None:
            d_emb = r_emb = o_emb = self.optic_enc(data['optic'], mask=mask)
        else:
            o_emb = data['optic']        
        ...
        if self.cross_enc is not None:
            # d_emb = r_emb = self.cross_enc(v_emb, o_emb, a_emb, mask=mask)
            d_emb = r_emb = self.cross_enc(v_emb, o_emb,mask=mask)

        if self.query_gen is not None:
            q_emb , q_mem = self.query_gen(r_emb, data.get('query'))
        ...
        for pred_head in self.pred_head:
            output = pred_head(d_emb, data, output, mode,video_feature =d_emb[-1],query_feature = q_mem)
